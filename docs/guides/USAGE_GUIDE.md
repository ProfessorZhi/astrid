...
│ Astrid                  │ provider                         │
│                                                                  │
│ Terminal coding assistant for Astrid.                        │
│                                                                  │
│ astrid                  │ .../Desktop/astrid/py-src        │
...
```bash
# 列出所有保存的会话
python -m astrid.main --list-sessions

# 恢复最近的会话
python -m astrid.main --resume

# 恢复特定会话
python -m astrid.main --resume abc123def456

# 使用特定会话 ID 启动
python -m astrid.main --session abc123def456
```
...
# 恢复会话
python -m astrid.main --resume abc123de
```
...
```bash
# 列出所有 MCP 服务器
python -m astrid.main mcp list

# 添加用户级服务器
python -m astrid.main mcp add myserver -- uvx my-mcp-server

# 添加项目级服务器
python -m astrid.main mcp add filesystem --project -- npx -y @modelcontextprotocol/server-filesystem .

# 移除服务器
python -m astrid.main mcp remove myserver
```
...
```bash
# 列出所有技能
python -m astrid.main skills list

# 添加技能
python -m astrid.main skills add ~/skills/frontend-dev --name frontend-dev

# 移除技能
python -m astrid.main skills remove frontend-dev
```
...
Astrid Python 是一个轻量级但功能完整的终端编码助手。
