...
| 文件 | 行数 | 功能 |
|------|------|------|
| `astrid/state.py` | 280 | Store 状态管理 + AppState |
| `astrid/cost_tracker.py` | 280 | 费用追踪 + 使用统计 |
| `astrid/tooling.py` | 扩展 | Tool Protocol + Metadata |
...
```python
from astrid.state import create_app_store, format_app_state_summary
...
# 更新状态
from astrid.state import set_busy, set_idle, update_context_usage
...
```python
from astrid.tooling import Tool, ToolMetadata, ToolCapability
...
```python
# tty_app.py 中已添加：
from astrid.state import AppState, Store, create_app_store
from astrid.cost_tracker import CostTracker
...
编辑 `src/astrid/cli/cli_commands.py`，添加：
...
编辑 `src/astrid/core/agent_loop.py`，在 API 调用后添加：
...
编辑 `src/astrid/ui/full/tty_app.py` 的 `on_tool_result` 回调：
...
| 维度 | Claude Code | Astrid Python (之前) | Astrid Python (现在) |
|------|-------------|----------------------|----------------------|
